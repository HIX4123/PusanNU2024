...
void WorkerThread::readWorker() {
  ...
  // std::cout << "read by key: " << &SharedPreferences::getInstance() << " " <<
  // size << " " << SharedPreferences::getInstance().getInt("key") << std::endl;
}
...
void WorkerThread::writeWorker(int value) {
  ...
  std::lock_guard<std::mutex> map_locker(mu);
  answers.emplace_back(&SharedPreferences::getInstance());
  // std::cout << "write by key: " << &SharedPreferences::getInstance() << " "
  // << size << " " << value << std::endl;
}

void WorkerThread::play() {
  WorkerThread workerThread;
  // std::cerr << "h/w cores: " << std::thread::hardware_concurrency() <<
  // std::endl;
  ...
  if (std::all_of(std::cbegin(answers), std::cend(answers), [=](const auto& e) {
        return e == &SharedPreferences::getInstance();
      }))
